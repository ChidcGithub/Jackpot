...
int level;
int ispre=1;                                      //Version
string pre="2.1",ver="1.16.211";             //Version
string preview="The error handling mechanism is incomplete, but does not affect normal use."; // Preview Detail
char choice;
...
int init(){
    ...
    cin>>level;
    if(level<1||level>2147483647){            //Has some problems...
		system("cls");
		try{
			cout<<0/0;                        //Must error!
		}
		catch(...){
			throw InputException();           //Has some problems...
		}
		cout<<1;
	}
